...
impl Solution {
    pub fn group_anagrams(strs: Vec<String>) -> Vec<Vec<String>> {
        strs.into_iter()
            .fold(
                HashMap::<[u8; ALPHABET_LENGTH], Vec<String>>::new(),
                |mut grouped_anagrams, word| {
                    grouped_anagrams
                        .entry(
                            word.bytes()
                                .map(|ordinal_character| (ordinal_character - ORDINAL_A) as usize)
                                .fold([0; ALPHABET_LENGTH], |mut frequency, character| {
